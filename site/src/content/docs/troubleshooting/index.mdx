...
**Cause:** The Discogs broker is unreachable or returned an unexpected error. Note that `REKLAWDBOX_DISCOGS_BROKER_URL` and `REKLAWDBOX_DISCOGS_BROKER_TOKEN` have compiled-in defaults pointing to the production broker — you do **not** need to set them unless you are self-hosting.

**Fix:** Check your network connectivity. If you are using a custom broker, verify that both `REKLAWDBOX_DISCOGS_BROKER_URL` and `REKLAWDBOX_DISCOGS_BROKER_TOKEN` are correct in your MCP config.
