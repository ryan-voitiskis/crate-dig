...
**Cause:** The broker URL is invalid. This should not occur with default configuration — the broker URL and token have compiled-in defaults pointing to the production broker. The most likely cause is overriding `REKLAWDBOX_DISCOGS_BROKER_URL` with an invalid URL in your MCP config.
