...
<Aside type="caution" title="Secrets in MCP config">
  The `REKLAWDBOX_DISCOGS_BROKER_TOKEN` is a secret. When using `.mcp.json`, ensure the file is listed in `.gitignore` so it is never committed to version control.
</Aside>
