...
4. **Runs all three providers concurrently:**
   ...
   - **Audio analysis** — scaled by `--cpu` preset: `background` (~50% cores, niced) or `overnight` (cores - 2, max throughput)
...
<Aside type="tip">
  Run `hydrate --cpu overnight -y` to warm your caches unattended. A library of 1,000 tracks takes roughly 30-60 minutes for Discogs + Beatport enrichment and 45-90 minutes for audio analysis. Use `--cpu background` (the default) if you're using the machine at the same time — it nices the process and uses fewer cores.
</Aside>
...
Default concurrency is set by the `--cpu` preset: `background` uses ~50% of cores with nice(10), `overnight` uses cores - 2 (clamped to [2, 16]). Cache validation checks file size and modification time — changed files are automatically re-analyzed.
