...
The `classify_tracks` tool applies the genre decision tree server-side using all cached evidence — Discogs styles, Beatport genre, label mappings, and audio analysis (BPM, danceability, spectral profile). Results are returned grouped by confidence level with a summary.
