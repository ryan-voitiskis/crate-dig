...
- stratum_dsp > 80% and enrichment > 60% → proceed
- Low coverage → recommend hydration first (audio analysis, then enrichment)
...
- `suggest #N` — find best replacement using `query_transition_candidates(from_track_id=prev, pool_track_ids=[remaining])`
