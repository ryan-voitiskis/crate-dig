...
For large scopes (full collection), inform user the first scan may take tens of seconds.
...
- For `FILENAME_TAG_DRIFT`: compare `detail.filename` vs `detail.tag` values (see character substitution handling below)
- For imported files needing rename: defer with note about manual Rekordbox relocate

**FILENAME_TAG_DRIFT — character substitution handling:**

Download tools often substitute characters in filenames. On macOS, only `/` and null are forbidden, but tools still substitute for portability:
...
2. If differences are only character substitutions from the table above, the **tag value is authoritative** — resolve as `accepted_as_is`
3. If real drift (different names, truncation, reordering), look up via `lookup_discogs` / `lookup_beatport`
4. If ambiguous after lookup, flag for user review — do not guess
...
| `MISSING_ALBUM` | Review | `album` field empty/null | Directory name or Discogs |
| `MISSING_YEAR` | Review | `year`/`date` both empty/null | Discogs lookup |
...
| `MISSING_YEAR_IN_DIR` | Review | Album directory missing `(YYYY)` suffix | Discogs lookup |
| `FILENAME_TAG_DRIFT` | Review | Filename artist/title disagrees with tags | Agent classifies substitution vs real drift |
